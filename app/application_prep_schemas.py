...
審査機関ごとに申請様式が異なるため、申請資格・申請先・申請方法・利用する様式体系を
明示判断として保持する。JIPDECオンラインと郵送・持参の様式差も区別する。
単なる「申請可能」フラグではなく、提出前に説明できる事実を記録する。
"""
...
class ApplicationPreparationState:
    ...
    # STEP2: 申請様式
    business_overview_prepared: bool = False      # JIPDECオンライン 新規 様式4
    office_list_prepared: bool = False            # JIPDECオンライン 新規 様式5
    pms_document_list_prepared: bool = False      # JIPDECオンライン 新規 様式6
    education_summary_prepared: bool = False      # JIPDECオンライン 新規 様式7
    audit_mr_summary_prepared: bool = False       # JIPDECオンライン 新規 様式8
    jipdec_mail_form_set_prepared: bool = False   # JIPDEC郵送・持参 新規申請書類一式
